...
describe('toPascalCase', function () {
	...
	test('transforms snake_cased word', function () {
		assert.equal(
			toPascalCase('arbok_rettan_pokemon'),
			'ArbokRettanPokemon',
		);
	});
	test('transforms kebab-cased word', function () {
		assert.equal(
			toPascalCase('black-blood-black-tears'),
			'BlackBloodBlackTears',
		);
	});
});
...
describe('toKebabCase', function () {
	...
	test('transforms snake_cased word', function () {
		assert.equal(
			toKebabCase('arbok_rettan_pokemon'),
			'arbok-rettan-pokemon',
		);
	});
	test('transforms PascalCased word', function () {
		assert.equal(
			toKebabCase('BlairePascalDidSomethingWithPhysics'),
			'blaire-pascal-did-something-with-physics',
		);
	});
});
